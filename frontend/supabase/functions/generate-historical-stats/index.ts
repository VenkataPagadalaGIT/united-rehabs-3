...
const RATE_LIMIT_MAX_REQUESTS = 10; // Lower limit for expensive batch operations
...
serve(async (req) => {
  ...
  try {
    ...
    // Check rate limit
    const rateLimitResult = await checkRateLimit(authResult.userId, "generate-historical-stats");
